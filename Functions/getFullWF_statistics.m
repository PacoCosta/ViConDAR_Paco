        ...
        if strcmp(AllWf{2,iWF},'original') % save only the stuff for the original if this is an orginal field
            ...
            for iY = 1:length(gridy)
                for iZ = 1:length(gridz)
                    ...
                    [U_coh.coh(iY,iZ,:), U_coh.f(iY,iZ,:)]        = mscohere(OriginalWF.U(iY,find(ismembertol(StatisticsWF.Original.time ,StatisticsWF.Constrained.time)),iZ),compU(iY,:,iZ),round(size(compU,2)/20),[],[],1/(StatisticsWF.Constrained.time(2)-StatisticsWF.Constrained.time(1)));
